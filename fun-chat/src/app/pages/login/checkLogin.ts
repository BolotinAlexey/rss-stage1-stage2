...
export default function checkLogin(e: Event): void {
  ...
  const isCorrectToSubmit = Object.values(nodeList).reduce((a: boolean, b) => {
    if (b instanceof HTMLInputElement) return a && b.classList.contains('correct') && !!b.value.length;
    return a;
  }, true);
  const btn = getElementDocument('.login__input-submit');
  if (!(btn instanceof HTMLButtonElement)) throw new Error('Not HTMLInputElement');
