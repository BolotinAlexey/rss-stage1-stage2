...
export default function isCheckInput(el: HTMLInputElement): boolean {
  ...
  if (el.classList.contains('login__input-name')) {
    if (val.length < 4) throw new Error('The letters number of name must be more then 4');
    if (!val.split('').every((char) => ALPHA_UPPER.includes(char.toUpperCase())))
      throw new Error('The letters must be in the English alphabet');
    if (val[0].toUpperCase() !== val[0]) throw new Error('The first letter of name should be upper case');
  } else {
    if (val.length < 4) throw new Error('The letters number of password must be more then 4');
